(() => {
  ...
  function overviewBusinessMetrics(performance) {
    ...
    return `<div class="business-metrics" aria-label="Performance della rete">${performance.metrics.map((metric,index) => `<a class="business-metric business-metric-${index+1} ${metric.note ? "is-pending" : ""}" href="${index === 0 || index === 2 ? "#fatturato-dealer" : "#analisi-aree"}" aria-label="Apri il dettaglio di ${escapeHtml(descriptions[metric.code] || metric.name)}"><div class="business-metric-head"><span class="business-metric-icon" aria-hidden="true"><span class="static-icon">${icon(metricIcons[metric.code] || "analysis")}</span></span><span class="business-metric-label">${descriptions[metric.code] || escapeHtml(metric.name)}</span></div><strong>${formatPerformanceValue(metric)}</strong><small>${metric.note ? escapeHtml(metric.note) : `Media dealer ${formatPerformanceValue(metric,metric.average)}`}</small></a>`).join("")}</div>`;
  }
