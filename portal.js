(() => {
  ...
  function overviewCompleteDialogs(performance,alertsAll,recentAll) {
    ...
    return `<dialog id="overview-leaders-dialog" class="review-dialog analysis-dealer-dialog"><div class="review-dialog-header"><div><p class="eyebrow">Classifica completa</p><h2>Dealer per fatturato ricambi</h2><p>${performance.dealerComparison.filter((item)=>item.value !== null).length} su ${performance.dealerComparison.length} dealer con un valore disponibile.</p></div><button type="button" data-close-dialog aria-label="Chiudi">×</button></div><div class="analysis-dealer-full-head" aria-hidden="true"><span>Rank</span><span>Concessionario</span><span>Valore</span><span>Confronto</span><span>Stato</span></div><ol class="analysis-dealer-full-list">${revenueRows}</ol></dialog>
      <dialog id="overview-priority-dialog" class="review-dialog overview-complete-dialog"><div class="review-dialog-header"><div><p class="eyebrow">Elenco completo</p><h2>Da completare o verificare</h2><p>${alertsAll.length} concessionari richiedono un’azione o il completamento dei dati.</p></div><button type="button" data-close-dialog aria-label="Chiudi">×</button></div><div class="overview-complete-head" aria-hidden="true"><span>Concessionario</span><span>Avanzamento</span><span>Stato</span></div><ul class="overview-complete-list">${priorityRows}</ul></dialog>
      <dialog id="overview-recent-dialog" class="review-dialog overview-complete-dialog"><div class="review-dialog-header"><div><p class="eyebrow">Elenco completo</p><h2>Dati ricevuti</h2><p>${recentAll.length} concessionari hanno inviato dati nella rilevazione corrente.</p></div><button type="button" data-close-dialog aria-label="Chiudi">×</button></div><div class="overview-complete-head" aria-hidden="true"><span>Concessionario</span><span>Ultimo aggiornamento</span><span>Stato</span></div><ul class="overview-complete-list">${recentRows}</ul></dialog>`;
  }
