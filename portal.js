(() => {
  ...
  function portalAnalysisPage() {
    ...
    return `<section class="page analysis-page analysis-design-page" aria-labelledby="page-title">
      ...
      <section class="analysis-summary-strip" aria-label="Riepilogo del KPI selezionato"><article class="analysis-summary-primary"><div class="analysis-summary-heading"><span class="analysis-summary-icon">${icon("analysis")}</span><span><small>KPI selezionato</small><strong>${escapeHtml(data.kpi.name)}</strong></span></div><div class="analysis-summary-value"><span>Media della rete</span><strong>${analysisValue(data.stats.average)}</strong><small>${escapeHtml(data.campaign.name)} · ${data.stats.count} rilevazioni valide</small></div></article><article><div class="analysis-stat-heading"><span class="analysis-stat-icon">${icon("median")}</span><span>Mediana</span></div><strong>${analysisValue(data.stats.median)}</strong><small>Valore centrale del campione</small></article><article><div class="analysis-stat-heading"><span class="analysis-stat-icon">${icon("trendDown")}</span><span>Minimo</span></div><strong>${analysisValue(data.stats.min)}</strong><small>Valore più basso osservato</small></article><article><div class="analysis-stat-heading"><span class="analysis-stat-icon">${icon("trendUp")}</span><span>Massimo</span></div><strong>${analysisValue(data.stats.max)}</strong><small>Valore più alto osservato</small></article></section>
      <div class="analysis-results-grid"><article class="panel analysis-regions"><div class="panel-header"><div class="analysis-panel-heading"><span class="analysis-panel-icon" aria-hidden="true">${icon("analysis")}</span><div><p class="eyebrow">Confronto tra regioni</p><h2>Media KPI per regione</h2><p>Ogni punto è la media dei dealer della regione, non un singolo dealer.</p></div></div><span class="analysis-unit">${escapeHtml(data.kpi.unit)}</span></div><div class="panel-body analysis-chart-body"><div class="analysis-chart-summary"><div class="analysis-leading-region"><span>Migliore media regionale</span><strong>${topRegion?escapeHtml(topRegion.region):"—"}</strong><small>${topRegion?analysisValue(topRegion.average):"Nessun dato disponibile"}${regionalDelta===null?"":`<em>${regionalDelta>=0?"+":""}${regionalDelta.toFixed(1).replace(".",",")}% sulla media di tutti i dealer</em>`}</small></div><div class="analysis-chart-context"><dl class="analysis-chart-metrics"><div><dt><span aria-hidden="true">${icon("trendUp")}</span>Ampiezza</dt><dd>${analysisValue(data.stats.max-data.stats.min)}</dd></div><div><dt><span aria-hidden="true">${icon("users")}</span>Campione</dt><dd>${data.stats.count} dealer</dd></div></dl><div class="analysis-region-legend"><span><i class="region-fill"></i>Media dealer della regione</span><span><i class="network-marker"></i>Media di tutti i dealer</span></div></div></div><div class="analysis-chart-scroll"><svg class="analysis-region-chart" viewBox="0 0 ${chartWidth} 330" role="img" aria-label="Media di ${escapeHtml(data.kpi.name)} dei dealer, raggruppata per regione"><defs><linearGradient id="analysisRegionFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#f36a15" stop-opacity=".16"/><stop offset="1" stop-color="#f36a15" stop-opacity=".015"/></linearGradient></defs>${chartTicks.map((tick)=>`<line class="analysis-chart-grid" x1="${chartLeft}" x2="${chartRight}" y1="${chartY(tick)}" y2="${chartY(tick)}"/><text class="analysis-chart-y-label" x="${chartLeft-12}" y="${chartY(tick)+4}" text-anchor="end">${chartAxisValue(tick)}</text>`).join("")}<path class="analysis-chart-area" d="${chartArea}"/><line class="analysis-chart-average" x1="${chartLeft}" x2="${chartRight}" y1="${chartY(networkAverage)}" y2="${chartY(networkAverage)}"/><text class="analysis-chart-average-label" x="${chartRight}" y="${chartY(networkAverage)-9}" text-anchor="end">Media di tutti i dealer · ${chartAxisValue(networkAverage)}</text><path class="analysis-chart-line" d="${chartLine}"/>${chartPoints.map((point,index)=>`<g><circle class="analysis-chart-dot-halo" cx="${point.x}" cy="${point.y}" r="9"/><circle class="analysis-chart-dot" cx="${point.x}" cy="${point.y}" r="4"/><text class="analysis-chart-x-label" x="${point.x}" y="${chartBottom+30}" text-anchor="middle">${escapeHtml(point.region)}</text>${index===0?`<text class="analysis-chart-value-label" x="${point.x}" y="${point.y-18}" text-anchor="start">${analysisValue(point.average)}</text>`:""}</g>`).join("")}</svg></div><p class="analysis-chart-note">Le regioni sono ordinate per la media dei rispettivi dealer. La linea tratteggiata indica la media calcolata su tutti i dealer del campione.</p></div></article>
        <article class="panel analysis-ranking"><div class="panel-header"><div class="analysis-panel-heading"><span class="analysis-panel-icon" aria-hidden="true">${icon("dealers")}</span><div><p class="eyebrow">Classifica singoli dealer</p><h2>Valori per concessionario</h2><p>Valori individuali; la regione indicata è la sede del dealer.</p></div></div></div><div class="panel-body"><ol class="analysis-ranking-list">${data.ranking.slice(0,5).map((item,index) => `<li class="${index===0?"is-leader":""}"><span class="analysis-ranking-position">${String(index+1).padStart(2,"0")}</span><span><strong>${escapeHtml(item.name)}</strong><small>Regione ${escapeHtml(item.region)}${index===0?'<em>1° valore individuale</em>':""}</small></span><strong>${analysisValue(item.value)}</strong></li>`).join("")}</ol></div></article></div>
    </section>`;
  }
