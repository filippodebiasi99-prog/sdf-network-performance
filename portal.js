(() => {
  ...
  function portalAnalysisPage() {
    ...
    const regionalBarWidth = (value) => Math.max(4,Math.min(100,(Number(value)||0)/regionalMax*100));
    ...
    return `<section class="page analysis-page analysis-design-page" aria-labelledby="page-title">
      ...
      <div class="analysis-results-grid"><article class="panel analysis-regions"><div class="panel-header"><div class="analysis-panel-heading"><span class="analysis-panel-icon" aria-hidden="true">${icon("analysis")}</span><div><p class="eyebrow">Distribuzione territoriale</p><h2>Media KPI per regione</h2><p>Regioni ordinate in base alla media dei rispettivi dealer.</p></div></div><span class="analysis-unit">${escapeHtml(data.kpi.unit)}</span></div><div class="panel-body analysis-region-body"><div class="analysis-region-overview"><div class="analysis-leading-region"><span>Migliore media regionale</span><strong>${topRegion?escapeHtml(topRegion.region):"—"}</strong><small>${topRegion?analysisValue(topRegion.average):"Nessun dato disponibile"}${regionalDelta===null?"":`<em>↑ ${regionalDelta>=0?"+":""}${regionalDelta.toFixed(1).replace(".",",")}% vs media rete</em>`}</small></div><dl class="analysis-region-metrics"><div><dt><span aria-hidden="true">${icon("trendUp")}</span>Ampiezza regionale</dt><dd>${analysisValue(regionalSpread)}</dd></div><div><dt><span aria-hidden="true">${icon("users")}</span>Campione</dt><dd>${data.stats.count} dealer</dd></div></dl></div><div class="analysis-region-scroll"><div class="analysis-region-ranking" role="table" aria-label="Classifica delle medie regionali per ${escapeHtml(data.kpi.name)}"><div class="analysis-region-table-head" role="row"><span role="columnheader">Rank</span><span role="columnheader">Regione</span><span aria-hidden="true"></span><span role="columnheader">Valore</span></div><div class="analysis-region-rows">${data.regions.map((item,index)=>`<div class="analysis-region-row ${index===0?"is-first":""}" role="row"><span class="analysis-region-rank" role="cell">${index+1}</span><strong class="analysis-region-name" role="cell">${escapeHtml(item.region)}</strong><span class="analysis-region-track" role="cell" aria-label="${analysisValue(item.average)}"><i style="width:${regionalBarWidth(item.average)}%"></i><b style="left:${networkMarker}%"></b></span><strong class="analysis-region-value" role="cell">${analysisValue(item.average)}</strong></div>`).join("")}</div><div class="analysis-region-network-label"><span style="left:${networkMarker}%">Media rete · ${analysisValue(networkAverage)}</span></div></div></div><p class="analysis-chart-note"><span aria-hidden="true">i</span>Ogni barra mostra la media dei dealer della regione. La linea tratteggiata rappresenta la media calcolata su tutti i dealer.</p></div></article>
