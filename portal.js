(() => {
  ...
  function portalAnalysisPage() {
    ...
    const regionBarWidth = (value) => relativeBarWidth(value,regionValues,22);
    const regionDelta = (value) => networkAverage ? (Number(value)/networkAverage-1)*100 : null;
    return `<section class="page analysis-page analysis-design-page" aria-labelledby="page-title">
      ...
      <div class="analysis-results-grid"><article class="panel analysis-regions"><div class="panel-header"><div class="analysis-panel-heading"><span class="analysis-panel-icon" aria-hidden="true">${icon("analysis")}</span><div><p class="eyebrow">Distribuzione territoriale</p><h2>Media KPI per regione</h2><p>Media dei dealer della regione, ordinata dal valore più alto.</p></div></div><span class="analysis-unit">${escapeHtml(data.kpi.unit)}</span></div><div class="panel-body analysis-region-list-body"><ol class="ranking-list analysis-region-ranking-list">${data.regions.map((item,index) => { const delta=regionDelta(item.average); return `<li><span class="ranking-position">${index+1}</span><div class="ranking-dealer"><strong>${escapeHtml(item.region)}</strong><small>${item.count} dealer</small></div><div class="ranking-value"><strong>${analysisValue(item.average)}</strong><small>${delta===null?"":`<b class="${delta>=0?"is-positive":"is-negative"}">${delta>=0?"+":""}${delta.toLocaleString("it-IT",{maximumFractionDigits:1})}%</b> <span>vs media rete</span>`}</small></div><span class="ranking-progress"><i style="width:${regionBarWidth(item.average)}%"></i></span></li>`; }).join("")}</ol><p class="analysis-chart-note">Il tratteggio completa la scala relativa tra la regione con la media più bassa e quella con la media più alta.</p></div></article>
