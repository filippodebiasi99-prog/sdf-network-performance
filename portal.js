(() => {
  const state = { config: null, overview: null, detail: null, analysis: null, campaigns: null, collection: null, collectionToken: null, online: false, poller: null, role:localStorage.getItem("sdf-demo-role") === "SDF" ? "SDF" : "JET", autosaveTimer:null,searchDealers:null,searchItems:[],searchActiveIndex:0,searchTrigger:null,dealerPage:1 };
  ...
  function completeListFilter(placeholder) {
    return `<label class="complete-list-filter">${icon("search")}<span class="sr-only">Filtra l’elenco</span><input type="search" data-complete-filter placeholder="${escapeHtml(placeholder)}" autocomplete="off"></label>`;
  }

  const completeListEmpty = '<p class="complete-list-empty" data-complete-filter-empty hidden>Nessun risultato corrisponde alla ricerca.</p>';

  function overviewCompleteDialogs(performance,alertsAll,recentAll) {
    const revenueRows=performance.dealerComparison.map((item) => `<li class="${item.value === null?"is-missing":""}"><span class="analysis-dealer-full-rank">${item.position?String(item.position).padStart(2,"0"):"—"}</span><span class="analysis-dealer-full-name"><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.region)} · ${escapeHtml(item.id)}</small></span><strong class="analysis-dealer-full-value">${item.value === null?"Dato non disponibile":formatPerformanceValue({kind:"currency"},item.value)}</strong><span class="analysis-dealer-full-delta ${item.deltaFromAverage === null?"":item.deltaFromAverage>=0?"is-positive":"is-negative"}">${item.deltaFromAverage === null?"—":`${item.deltaFromAverage>=0?"+":""}${item.deltaFromAverage.toLocaleString("it-IT",{maximumFractionDigits:1})}% vs media`}</span>${collectionStatusBadge(item.collection_status)}</li>`).join("");
    const priorityRows=alertsAll.map((item) => `<li><span class="overview-complete-name"><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.region)} · ${escapeHtml(item.id)}</small></span><span class="overview-complete-detail">${item.collection_status === "NOT_STARTED"?"Rilevazione non iniziata":item.collection_status === "DRAFT"?`Bozza al ${Math.round(item.completion || item.quality || 0)}%`:item.collection_status === "REOPENED"?"Compilazione riaperta":"Dati da verificare"}</span>${collectionStatusBadge(item.collection_status)}</li>`).join("");
    const recentRows=recentAll.map((item) => `<li><span class="overview-complete-name"><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.region)} · ${escapeHtml(item.id)}</small></span><time class="overview-complete-detail">${formatDate(item.updated_at)}</time>${collectionStatusBadge(item.collection_status)}</li>`).join("");
    return `<dialog id="overview-leaders-dialog" class="review-dialog analysis-dealer-dialog"><div class="review-dialog-header"><div><p class="eyebrow">Classifica completa</p><h2>Dealer per fatturato ricambi</h2><p>${performance.dealerComparison.filter((item)=>item.value !== null).length} su ${performance.dealerComparison.length} dealer con un valore disponibile.</p></div><button type="button" data-close-dialog aria-label="Chiudi">×</button></div>${completeListFilter("Cerca concessionario, regione, ID o stato")}<div class="analysis-dealer-full-head" aria-hidden="true"><span>Rank</span><span>Concessionario</span><span>Valore</span><span>Confronto</span><span>Stato</span></div><ol class="analysis-dealer-full-list">${revenueRows}</ol>${completeListEmpty}</dialog>
      <dialog id="overview-priority-dialog" class="review-dialog overview-complete-dialog"><div class="review-dialog-header"><div><p class="eyebrow">Elenco completo</p><h2>Da completare o verificare</h2><p>${alertsAll.length} concessionari richiedono un’azione o il completamento dei dati.</p></div><button type="button" data-close-dialog aria-label="Chiudi">×</button></div>${completeListFilter("Cerca concessionario, regione, ID o stato")}<div class="overview-complete-head" aria-hidden="true"><span>Concessionario</span><span>Avanzamento</span><span>Stato</span></div><ul class="overview-complete-list">${priorityRows}</ul>${completeListEmpty}</dialog>
      <dialog id="overview-recent-dialog" class="review-dialog overview-complete-dialog"><div class="review-dialog-header"><div><p class="eyebrow">Elenco completo</p><h2>Dati ricevuti</h2><p>${recentAll.length} concessionari hanno inviato dati nella rilevazione corrente.</p></div><button type="button" data-close-dialog aria-label="Chiudi">×</button></div>${completeListFilter("Cerca concessionario, regione, ID o stato")}<div class="overview-complete-head" aria-hidden="true"><span>Concessionario</span><span>Ultimo aggiornamento</span><span>Stato</span></div><ul class="overview-complete-list">${recentRows}</ul>${completeListEmpty}</dialog>`;
  }
  ...
  function portalOverviewPage() {
    ...
    const { campaign, totals, recent, alerts,recentAll=[],alertsAll=[],performance } = state.overview;
  ...
  function portalDealerResults(list) {
    ...
    state.dealerPage=Math.max(1,Math.min(state.dealerPage,pageCount));
    const start=(state.dealerPage-1)*pageSize;
    const visible=list.slice(start,start+pageSize);
    const pages=Array.from({length:pageCount},(_,index)=>index+1).map((page)=>`<button class="dealer-page-button ${page===state.dealerPage?"is-current":""}" type="button" data-dealer-page="${page}" ${page===state.dealerPage?'aria-current="page"':""}>${page}</button>`).join("");
    const controls=pageCount>1?`<button class="dealer-page-button" type="button" data-dealer-page="${state.dealerPage-1}" aria-label="Pagina precedente" ${state.dealerPage===1?"disabled":""}>‹</button>${pages}<button class="dealer-page-button" type="button" data-dealer-page="${state.dealerPage+1}" aria-label="Pagina successiva" ${state.dealerPage===pageCount?"disabled":""}>›</button>`:pages;
    const range=list.length?`${start+1}–${Math.min(start+pageSize,list.length)} di ${list.length}`:"0";
    return `<div class="table-wrap"><table class="dealers-table"><thead><tr><th>Concessionario</th><th>Campagna</th><th>Stato</th><th>Ultimo invio</th><th>Azioni</th></tr></thead><tbody>${visible.map((dealer) => `<tr data-dealer-row="${escapeHtml(dealer.id)}"><td><button class="dealer-name dealer-link" data-dealer-id="${escapeHtml(dealer.id)}">${escapeHtml(dealer.name)}<small>${escapeHtml(dealer.id)} · ${escapeHtml(dealer.region)}</small></button></td><td>${escapeHtml(state.overview.campaign.name)}</td><td>${collectionStatusBadge(dealer.collection_status)}</td><td>${formatDate(dealer.submitted_at,true)}</td><td><div class="dealer-row-actions row-actions">${state.role === "JET"?`<button class="button compact" data-copy-link="${escapeHtml(dealer.id)}">Copia link</button><button class="button compact" data-show-qr="${escapeHtml(dealer.id)}">QR</button>`:""}<button class="dealer-open row-action" data-dealer-id="${escapeHtml(dealer.id)}" aria-label="Apri la scheda di ${escapeHtml(dealer.name)}"><span>Apri scheda</span>${icon("chevron")}</button></div></td></tr>`).join("")}</tbody></table></div><footer class="dealer-pagination"><span>Visualizzati ${range} concessionari</span><nav class="dealer-page-controls" aria-label="Paginazione concessionari">${controls}</nav></footer>`;
  }

  function bindDealerScrollHandoff() {
    const scroller=main.querySelector(".dealers-table-panel #dealer-results .table-wrap");
    if (!scroller || scroller.dataset.scrollHandoff) return;
    scroller.dataset.scrollHandoff="true";
    const pageAtBottom=()=>window.scrollY+window.innerHeight>=document.documentElement.scrollHeight-2;
    const handoff=(delta,event)=>{
      if ((delta>0&&!pageAtBottom()) || (delta<0&&scroller.scrollTop<=0)) {
        event.preventDefault();
  ...
  function portalAnalysisPage() {
    ...
    const visibleRegions = data.regions.slice(0,8);
    const hasMoreRegions = data.regions.length > visibleRegions.length;
    ...
    const dealerComparison = data.dealerComparison || [];
    const dealersWithValue = dealerComparison.filter((dealer) => dealer.value !== null).length;
    const dealerComparisonRows = dealerComparison.map((dealer) => {
      const delta=dealer.value === null ? null : dealerDelta(dealer.value);
      return `<li class="${dealer.value === null?"is-missing":""}"><span class="analysis-dealer-full-rank">${dealer.rank?String(dealer.rank).padStart(2,"0"):"—"}</span><span class="analysis-dealer-full-name"><strong>${escapeHtml(dealer.name)}</strong><small>${escapeHtml(dealer.region)} · ${escapeHtml(dealer.id)}</small></span><strong class="analysis-dealer-full-value">${dealer.value === null?"Dato non disponibile":analysisValue(dealer.value)}</strong><span class="analysis-dealer-full-delta ${delta === null?"":delta>=0?"is-positive":"is-negative"}">${delta === null?"—":`${delta>=0?"+":""}${delta.toLocaleString("it-IT",{maximumFractionDigits:1})}% vs media`}</span>${collectionStatusBadge(dealer.collection_status)}</li>`;
    }).join("");
    const regionComparisonRows = data.regions.map((item,index) => {
      const delta=networkAverage ? (Number(item.average)/networkAverage-1)*100 : null;
      return `<li><span class="analysis-dealer-full-rank">${String(index+1).padStart(2,"0")}</span><strong class="analysis-region-full-name">${escapeHtml(item.region)}</strong><span class="analysis-region-full-sample">${item.count} dealer</span><strong class="analysis-region-full-value">${analysisValue(item.average)}</strong><span class="analysis-dealer-full-delta ${delta === null?"":delta>=0?"is-positive":"is-negative"}">${delta === null?"—":`${delta>=0?"+":""}${delta.toLocaleString("it-IT",{maximumFractionDigits:1})}% vs rete`}</span></li>`;
    }).join("");
  ...
  function bindFunctionalEvents() {
    ...
    bindDealerScrollHandoff();
    const dealerResults=main.querySelector("#dealer-results");
    ...
    const filteredDealers=()=>{
      ...
      return dealers.filter((dealer) => (!term || `${dealer.name} ${dealer.id}`.toLowerCase().includes(term)) && (!region || dealer.region === region) && (!status || dealer.collection_status === status));
    };
    const renderDealerResults=(filtered)=>{
      if (!dealerResults) return;
      dealerResults.innerHTML=portalDealerResults(filtered);
      hydrateIcons(dealerResults);
      bindDealerScrollHandoff();
    };
    const portalFilter = () => {
      const search = main.querySelector("#dealer-search");
      if (!search || !dealerResults) return;
      const term = search.value.trim().toLowerCase();
      const region = main.querySelector("#region-filter")?.value || "";
      const status = main.querySelector("#status-filter")?.value || "";
      const filtered=filteredDealers();
      const filterSummary=main.querySelector("#dealer-filter-summary");
      if (filterSummary) filterSummary.textContent=[`${filtered.length} dealer`,`${new Set(filtered.map((dealer)=>dealer.region)).size} regioni`,term?`Ricerca: “${search.value.trim()}”`:null,region?`Regione: ${region}`:null,status?`Stato: ${statusLabel[status] || status}`:"Tutti gli stati"].filter(Boolean).join(" · ");
      state.dealerPage=1;
    ...
    dealerResults?.addEventListener("click",async(event)=>{
      const pageButton=event.target.closest("[data-dealer-page]");
      if (pageButton) { state.dealerPage=Number(pageButton.dataset.dealerPage); renderDealerResults(filteredDealers()); return; }
      const copyButton=event.target.closest("[data-copy-link]");
      if (copyButton) {
        try { const link=await getLink(copyButton.dataset.copyLink); await navigator.clipboard.writeText(link.url); showToast("Link di compilazione copiato."); }
        catch (error) { showToast(error.message); }
        return;
      }
      const qrButton=event.target.closest("[data-show-qr]");
      if (qrButton) {
        try {
        const link=await getLink(qrButton.dataset.showQr); const dealer=dealers.find((item)=>item.id===qrButton.dataset.showQr);
        const dialog=main.querySelector("#qr-dialog");
        dialog.querySelector("#qr-dialog-content").innerHTML=`<p class="eyebrow">Link concessionario</p><h2>${escapeHtml(dealer?.name || qrButton.dataset.showQr)}</h2><img src="${escapeHtml(link.qrUrl)}" alt="QR Code"><code>${escapeHtml(link.url)}</code><div class="inline-actions"><a class="button primary" href="${escapeHtml(link.qrUrl)}" download="qr-${escapeHtml(qrButton.dataset.showQr)}.svg">Scarica SVG</a><button class="button" id="qr-copy">Copia link</button><button class="button" id="qr-print">Stampa</button></div>`;
      ...
      const dealerButton=event.target.closest("[data-dealer-id]");
      const row=event.target.closest("[data-dealer-row]");
      if (!dealerButton && (!row || event.target.closest("button,a,input,select,label"))) return;
      const dealerId=dealerButton?.dataset.dealerId || row.dataset.dealerRow;
      selectedDealer=dealers.find((dealer)=>dealer.id===dealerId) || {id:dealerId};
    ...
    main.querySelector("#open-analysis-dealers")?.addEventListener("click",()=>main.querySelector("#analysis-dealer-dialog")?.showModal());
    main.querySelector("#open-analysis-regions")?.addEventListener("click",()=>main.querySelector("#analysis-region-dialog")?.showModal());
    [["open-overview-leaders","overview-leaders-dialog"],["open-overview-priority","overview-priority-dialog"],["open-overview-recent","overview-recent-dialog"]].forEach(([buttonId,dialogId])=>main.querySelector(`#${buttonId}`)?.addEventListener("click",()=>main.querySelector(`#${dialogId}`)?.showModal()));
    main.querySelectorAll("[data-complete-filter]").forEach((input)=>{
      const dialog=input.closest("dialog");
      const rows=[...(dialog?.querySelectorAll("ol li, ul li") || [])];
      const empty=dialog?.querySelector("[data-complete-filter-empty]");
      const applyCompleteFilter=()=>{
        const term=normalizeSearch(input.value);
        let visible=0;
        rows.forEach((row)=>{
          row.hidden=Boolean(term) && !normalizeSearch(row.textContent).includes(term);
          if (!row.hidden) visible+=1;
        });
        if (empty) empty.hidden=visible>0;
      };
      input.addEventListener("input",applyCompleteFilter);
