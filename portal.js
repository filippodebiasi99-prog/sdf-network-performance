(() => {
  ...
  function portalAnalysisPage() {
    ...
    const averageMarker = Math.min(100,Math.max(0,networkAverage / max * 100));
    ...
    return `<section class="page analysis-page" aria-labelledby="page-title">${pageHeader({ eyebrow:"Benchmark e distribuzioni",title:'<span id="page-title">Analisi KPI</span>',subtitle:"Confronta risultati, distribuzione territoriale e concessionari sullo stesso indicatore.",actions:`<button class="button" data-export-csv>${icon("download")}Esporta dati</button>` })}
      ...
      <div class="analysis-results-grid"><article class="panel analysis-regions"><div class="panel-header"><div><p class="eyebrow">Distribuzione territoriale</p><h2>Confronto per regione</h2><p>La linea verticale indica la media complessiva della rete.</p></div><span class="analysis-unit">${escapeHtml(data.kpi.unit)}</span></div><div class="panel-body"><div class="analysis-region-legend"><span><i class="region-fill"></i>Media regionale</span><span><i class="network-marker"></i>Media rete</span></div><ol class="analysis-region-list">${data.regions.map((item,index) => `<li><span class="analysis-region-rank">${String(index+1).padStart(2,"0")}</span><span class="analysis-region-name"><strong>${escapeHtml(item.region)}</strong><small>${Number(item.average)>=networkAverage?"Sopra":"Sotto"} la media rete</small></span><div class="analysis-region-track" role="img" aria-label="${escapeHtml(item.region)}: ${analysisValue(item.average)}"><span style="width:${Math.max(2,item.average/max*100)}%"></span><i style="left:${averageMarker}%"></i></div><strong class="analysis-region-value">${analysisValue(item.average)}</strong></li>`).join("")}</ol></div></article>
