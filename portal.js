(() => {
  ...
  function overviewBusinessMetrics(performance) {
    ...
    return `<div class="business-metrics" aria-label="Performance della rete">${performance.metrics.map((metric) => `<article class="business-metric ${metric.note ? "is-pending" : ""}"><span>${descriptions[metric.code] || escapeHtml(metric.name)}</span><strong>${formatPerformanceValue(metric)}</strong><small>${metric.note ? escapeHtml(metric.note) : `Media dealer ${formatPerformanceValue(metric,metric.average)}`}</small></article>`).join("")}</div>`;
  }
