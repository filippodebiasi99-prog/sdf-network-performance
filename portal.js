(() => {
  ...
  function portalAnalysisPage() {
    ...
    const dealerBarWidth = (value) => relativeBarWidth(value,dealerValues,22);
    ...
    return `<section class="page analysis-page analysis-design-page" aria-labelledby="page-title">
      ...
      <div class="analysis-results-grid"><article class="panel analysis-regions"><div class="panel-header"><div class="analysis-panel-heading"><span class="analysis-panel-icon" aria-hidden="true">${icon("analysis")}</span><div><p class="eyebrow">Distribuzione territoriale</p><h2>Media KPI per regione</h2><p>Medie regionali ordinate dal valore più alto.</p></div></div><span class="analysis-unit">${escapeHtml(data.kpi.unit)}</span></div><div class="panel-body analysis-region-list-body"><div class="analysis-region-table-scroll"><div class="analysis-region-table-head" aria-hidden="true"><span>Rank</span><span>Regione</span><span></span><span>Valore</span></div><ol class="analysis-region-compact-list" aria-label="Classifica delle medie regionali">${data.regions.map((item,index) => `<li><span class="ranking-position">${index+1}</span><strong class="analysis-region-name">${escapeHtml(item.region)}</strong><span class="ranking-progress" aria-label="${analysisValue(item.average)}"><i style="width:${regionBarWidth(item.average)}%"></i><b style="left:${networkMarker}%" aria-hidden="true"></b></span><strong class="analysis-region-value">${analysisValue(item.average)}</strong></li>`).join("")}</ol><div class="analysis-network-average-row"><span style="left:${networkMarker}%">Media rete · ${analysisValue(networkAverage)}</span></div></div><p class="analysis-chart-note">Le barre confrontano le medie dei dealer di ciascuna regione; la linea tratteggiata indica la media complessiva della rete.</p></div></article>
        <div class="analysis-secondary-grid"><article class="panel analysis-ranking"><div class="panel-header"><div class="analysis-panel-heading"><span class="analysis-panel-icon" aria-hidden="true">${icon("dealers")}</span><div><p class="eyebrow">Classifica singoli dealer</p><h2>Valori per concessionario</h2><p>Valori individuali; la regione indicata è la sede del dealer.</p></div></div></div><div class="panel-body analysis-dealer-list-body"><ol class="ranking-list analysis-dealer-ranking-list">${topDealers.map((item,index) => { const delta=dealerDelta(item.value); return `<li><span class="ranking-position">${String(index+1).padStart(2,"0")}</span><div class="ranking-dealer"><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.region)}</small></div><div class="ranking-value"><strong>${analysisValue(item.value)}</strong><small>${delta===null?"":`<b class="${delta>=0?"is-positive":"is-negative"}">${delta>=0?"+":""}${delta.toLocaleString("it-IT",{maximumFractionDigits:1})}%</b> <span>vs media</span>`}</small></div><span class="ranking-progress"><i style="width:${dealerBarWidth(item.value)}%"></i></span></li>`; }).join("")}</ol></div></article><article class="panel analysis-region-insights"><div class="panel-header"><div class="analysis-panel-heading"><span class="analysis-panel-icon" aria-hidden="true">${icon("trendUp")}</span><div><p class="eyebrow">Lettura del confronto</p><h2>Scenario regionale</h2><p>Indicatori sintetici della distribuzione territoriale.</p></div></div></div><div class="panel-body analysis-region-insights-body"><div class="analysis-region-leader"><span>Regione in testa</span><strong>${topRegion?escapeHtml(topRegion.region):"—"}</strong><div><b>${topRegion?analysisValue(topRegion.average):"Nessun dato"}</b>${regionalDelta===null?"":`<em>↑ ${regionalDelta>=0?"+":""}${regionalDelta.toLocaleString("it-IT",{maximumFractionDigits:1})}% vs rete</em>`}</div></div><dl class="analysis-region-insight-metrics"><div><dt>Ampiezza regionale</dt><dd>${analysisValue(regionalSpread)}</dd></div><div><dt>Campione</dt><dd>${data.stats.count} dealer</dd></div></dl><p>Le medie regionali aggregano i valori dei dealer della stessa regione. La classifica a fianco mostra invece i singoli concessionari.</p></div></article></div></div>
    </section>`;
  }
