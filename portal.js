(() => {
  ...
  function overviewBusinessMetrics(performance) {
    const descriptions={ company_revenue_total:"Fatturato complessivo",parts_margin_average:"Marginalità media ricambi",parts_revenue_total:"Ricavi ricambi",inventory_turnover:"Indice di rotazione medio",inventory_end_value:"Tot stock magazzini rete Italia" };
    const metricIcons={ company_revenue_total:"reports",parts_margin_average:"analysis",parts_revenue_total:"overview",inventory_turnover:"calendar",inventory_end_value:"dealers" };
    return `<div class="business-metrics" aria-label="Performance della rete">${performance.metrics.map((metric,index) => `<article class="business-metric business-metric-${index+1} ${metric.note ? "is-pending" : ""}"><div class="business-metric-head"><span class="business-metric-icon" aria-hidden="true">${icon(metricIcons[metric.code] || "analysis")}</span><span class="business-metric-label">${descriptions[metric.code] || escapeHtml(metric.name)}</span></div><strong>${formatPerformanceValue(metric)}</strong><small>${metric.note ? escapeHtml(metric.note) : `Media dealer ${formatPerformanceValue(metric,metric.average)}`}</small></article>`).join("")}</div>`;
  }
  ...
  function portalSubmissionChart() {
    ...
    return `<svg class="line-chart" viewBox="0 0 700 222" role="img" aria-label="Andamento cumulativo delle compilazioni fino a ${state.overview.totals.received} invii."><defs><linearGradient id="liveChartGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#da8d1b" stop-opacity=".2"/><stop offset="1" stop-color="#da8d1b" stop-opacity="0"/></linearGradient></defs><path class="chart-grid-line" d="M40 24H680M40 68H680M40 112H680M40 156H680M40 200H680"/><text class="chart-label" x="10" y="203">0</text><text class="chart-label" x="5" y="159">${Math.round(maximum*.25)}</text><text class="chart-label" x="5" y="115">${Math.round(maximum*.5)}</text><text class="chart-label" x="5" y="71">${Math.round(maximum*.75)}</text><text class="chart-label" x="5" y="27">${maximum}</text><path d="${area}" fill="url(#liveChartGradient)"/><path class="chart-line" d="${line}"/>${coordinates.map((point) => `<circle class="chart-dot" cx="${point[0]}" cy="${point[1]}" r="3"/>`).join("")}<text class="chart-label" x="40" y="218">${start}</text><text class="chart-label" x="640" y="218">${end}</text></svg>`;
  }
  ...
  function portalOverviewPage() {
    ...
    return `<section class="page overview-page" aria-labelledby="page-title">
      ${pageHeader({ eyebrow:"Overview · Rete concessionari", title:'<span id="page-title">Performance della rete</span>', subtitle:`Indicatori economici e andamento dei ${performance.sample} concessionari inclusi nel campione.`, actions:`<select class="select-compact" aria-label="Seleziona rilevazione">${state.config.campaigns.map((item) => `<option value="${item.id}" ${item.id === campaign.id ? "selected" : ""}>${item.name}</option>`).join("")}</select><button class="button" data-export-csv>${icon("download")}Esporta dati</button>` })}
      ${overviewBusinessMetrics(performance)}
      <div class="overview-analysis-grid"><article class="panel"><div class="panel-header"><div><h2>Dealer per fatturato ricambi</h2><p>Primi cinque concessionari · confronto con la media rete ricambi</p></div><button class="text-button" data-page-link="analysis">Analisi completa →</button></div><div class="panel-body">${overviewRevenueLeaders(performance)}</div></article><article class="panel"><div class="panel-header"><div><h2>Fatturato medio per area</h2><p>Benchmark relativo sui dati ricevuti</p></div><button class="text-button" data-page-link="analysis">Confronta →</button></div><div class="panel-body">${overviewAreaPerformance(performance)}</div></article></div>
  ...
  function portalAnalysisPage() {
    ...
    const topDealer = data.ranking[0];
    ...
    const observedMin = regionValues.length ? Math.min(...regionValues) : 0;
    const observedMax = regionValues.length ? Math.max(...regionValues) : 1;
    const observedRange = Math.max(observedMax-observedMin,Math.abs(observedMax)*.08,1);
    const chartFloor = Math.max(0,observedMin-observedRange*.18);
    const chartCeiling = observedMax+observedRange*.12;
    const chartY = (value) => chartBottom-((value-chartFloor)/(chartCeiling-chartFloor))*(chartBottom-chartTop);
    const chartPoints = data.regions.map((item,index) => ({...item,x:chartLeft+(index*(chartRight-chartLeft)/Math.max(1,data.regions.length-1)),y:chartY(Number(item.average)||0)}));
    const chartLine = chartPoints.reduce((path,point,index,points) => { if(index===0)return `M ${point.x} ${point.y}`;const previous=points[index-1];const middle=(previous.x+point.x)/2;return `${path} C ${middle} ${previous.y}, ${middle} ${point.y}, ${point.x} ${point.y}`; },"");
    const chartArea = chartPoints.length ? `${chartLine} L ${chartPoints.at(-1).x} ${chartBottom} L ${chartPoints[0].x} ${chartBottom} Z` : "";
    const chartTicks = [chartCeiling,(chartCeiling+chartFloor)/2,chartFloor];
    const chartAxisValue = (value) => data.kpi.unit === "EUR" ? `€ ${(value/1_000_000).toLocaleString("it-IT",{maximumFractionDigits:1})} mln` : analysisValue(value);
    return `<section class="page analysis-page" aria-labelledby="page-title">${pageHeader({ eyebrow:"Benchmark e distribuzioni",title:'<span id="page-title">Analisi KPI</span>',subtitle:"Confronta risultati, distribuzione territoriale e concessionari sullo stesso indicatore.",actions:`<button class="button" data-export-csv>${icon("download")}Esporta dati</button>` })}
      ...
      <article class="analysis-spotlight"><div class="analysis-spotlight-copy"><div><p class="eyebrow">KPI selezionato</p><h2>${escapeHtml(data.kpi.name)}</h2><p>${escapeHtml(data.campaign.name)} · ${data.stats.count} rilevazioni valide</p></div><span class="analysis-live-status"><i></i>Dati aggiornati</span></div><div class="analysis-primary-value"><span>Media della rete</span><strong>${analysisValue(data.stats.average)}</strong><small>Unità di misura · ${escapeHtml(data.kpi.unit)}</small></div><dl class="analysis-key-stats"><div><dt>Mediana</dt><dd>${analysisValue(data.stats.median)}</dd></div><div><dt>Minimo</dt><dd>${analysisValue(data.stats.min)}</dd></div><div><dt>Massimo</dt><dd>${analysisValue(data.stats.max)}</dd></div></dl></article>
      <div class="analysis-results-grid"><article class="panel analysis-regions"><div class="panel-header"><div><p class="eyebrow">Distribuzione territoriale</p><h2>Curva regionale</h2><p>Regioni ordinate per performance sul KPI selezionato.</p></div><span class="analysis-unit">${escapeHtml(data.kpi.unit)}</span></div><div class="panel-body analysis-chart-body"><div class="analysis-chart-summary"><div><span>Regione in testa</span><strong>${topRegion?escapeHtml(topRegion.region):"—"}</strong><small>${topRegion?analysisValue(topRegion.average):"Nessun dato disponibile"}</small></div><div class="analysis-region-legend"><span><i class="region-fill"></i>Media regionale</span><span><i class="network-marker"></i>Media rete</span></div></div><div class="analysis-chart-scroll"><svg class="analysis-region-chart" viewBox="0 0 ${chartWidth} 330" role="img" aria-label="Distribuzione di ${escapeHtml(data.kpi.name)} per regione"><defs><linearGradient id="analysisRegionFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#da8d1b" stop-opacity=".16"/><stop offset="1" stop-color="#da8d1b" stop-opacity=".015"/></linearGradient></defs>${chartTicks.map((tick)=>`<line class="analysis-chart-grid" x1="${chartLeft}" x2="${chartRight}" y1="${chartY(tick)}" y2="${chartY(tick)}"/><text class="analysis-chart-y-label" x="${chartLeft-12}" y="${chartY(tick)+4}" text-anchor="end">${chartAxisValue(tick)}</text>`).join("")}<path class="analysis-chart-area" d="${chartArea}"/><line class="analysis-chart-average" x1="${chartLeft}" x2="${chartRight}" y1="${chartY(networkAverage)}" y2="${chartY(networkAverage)}"/><text class="analysis-chart-average-label" x="${chartRight}" y="${chartY(networkAverage)-9}" text-anchor="end">Media rete · ${chartAxisValue(networkAverage)}</text><path class="analysis-chart-line" d="${chartLine}"/>${chartPoints.map((point,index)=>`<g><circle class="analysis-chart-dot-halo" cx="${point.x}" cy="${point.y}" r="9"/><circle class="analysis-chart-dot" cx="${point.x}" cy="${point.y}" r="4"/><text class="analysis-chart-x-label" x="${point.x}" y="${chartBottom+30}" text-anchor="middle">${escapeHtml(point.region)}</text>${index===0?`<text class="analysis-chart-value-label" x="${point.x}" y="${point.y-18}" text-anchor="start">${analysisValue(point.average)}</text>`:""}</g>`).join("")}</svg></div><p class="analysis-chart-note">La linea tratteggiata mostra la media complessiva; la curva evidenzia distanza e progressione tra le regioni.</p></div></article>
        <article class="panel analysis-ranking"><div class="panel-header"><div><p class="eyebrow">Top performance</p><h2>Migliori concessionari</h2><p>Ordinati per ${escapeHtml(data.kpi.name.toLowerCase())}.</p></div></div><div class="panel-body"><ol class="analysis-ranking-list">${data.ranking.slice(0,5).map((item,index) => `<li><span class="analysis-ranking-position">${String(index+1).padStart(2,"0")}</span><span><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.region)}</small></span><strong>${analysisValue(item.value)}</strong></li>`).join("")}</ol></div></article></div>
      <section class="analysis-insights" aria-label="Indicatori di lettura"><article><span>Regione più performante</span><strong>${topRegion?escapeHtml(topRegion.region):"—"}</strong><small>${regionalDelta===null?"Confronto non disponibile":`${regionalDelta>=0?"+":""}${regionalDelta.toFixed(1).replace(".",",")}% rispetto alla rete`}</small></article><article><span>Dealer in testa</span><strong>${topDealer?escapeHtml(topDealer.name):"—"}</strong><small>${topDealer?analysisValue(topDealer.value):"Nessun dato"}</small></article><article><span>Ampiezza osservata</span><strong>${analysisValue(data.stats.max-data.stats.min)}</strong><small>Differenza tra massimo e minimo</small></article><article><span>Copertura del campione</span><strong>${data.stats.count} dealer</strong><small>Rilevazioni valide analizzate</small></article></section>
    </section>`;
  }
