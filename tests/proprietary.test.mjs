...
before(async()=>{ await new Promise((resolve,reject)=>{server.once("error",reject);server.listen(0,"127.0.0.1",resolve)}); baseUrl=`http://127.0.0.1:${server.address().port}`; const link=await call("/api/dealers/IT-0042/collection-link?campaignId=campaign-2026-1").then(r=>r.json()); token=new URL(link.url).pathname.split("/").pop(); });
...
test("valid dealer link loads the centrally configured questionnaire",async()=>{ const response=await call(`/api/compila/${token}`); assert.equal(response.status,200); const payload=await response.json(); assert.equal(payload.mode,"proprietary"); assert.equal(payload.questionnaire.fields.length,20); assert.equal(payload.dealer.id,"IT-0042"); });
...
test("JET can reopen while SDF cannot mutate",async()=>{ let response=await call("/api/dealers/IT-0042/submission/status",{method:"POST",body:JSON.stringify({campaignId:"campaign-2026-1",status:"REOPENED"}),headers:{"x-demo-role":"SDF"}}); assert.equal(response.status,403); response=await call("/api/dealers/IT-0042/submission/status",{method:"POST",body:JSON.stringify({campaignId:"campaign-2026-1",status:"REOPENED"}),headers:{"x-demo-role":"JET"}}); assert.equal(response.status,200); assert.equal(database.prepare("SELECT collection_status FROM submissions WHERE dealer_id='IT-0042' AND campaign_id='campaign-2026-1'").get().collection_status,"REOPENED"); });
test("SDF dealer detail is read-only and excludes internal data",async()=>{ const detail=await call("/api/dealers/IT-0018",{headers:{"x-demo-role":"SDF"}}).then(r=>r.json()); assert.equal(detail.collectionLink,null); assert.deepEqual(detail.notes,[]); assert.equal(detail.jotform,null); });
