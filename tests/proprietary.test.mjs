...
test("questionnaire frontend includes debounced autosave, valid-field progress and mobile stacking",()=>{ const js=readFileSync(new URL("../portal.js",import.meta.url),"utf8"); const css=readFileSync(new URL("../styles.css",import.meta.url),"utf8"); assert.match(js,/setTimeout\(\(\)=>submitSurvey\("draft",\{silent:true\}\),1800\)/); assert.match(js,/dati completati · \$\{percent\}%/); assert.match(css,/@media \(max-width:640px\)/); assert.match(css,/\.questionnaire-shell \{ display:block;/); assert.match(css,/\.survey-field \{ grid-template-columns:1fr;/); });
